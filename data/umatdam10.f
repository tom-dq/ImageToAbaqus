...
C     Tom new - keep the old stress
      DIMENSION OLDSIG(NTENS)
...
C      -- Tom new - hold on to the original stress --
C     DOUBLE PRECISION, DIMENSION(ntens) :: OLDSIG
...
CTOM --- debug output
      DATAOUT = 0
      IF ((NOEL.GE.1007397).AND.(NOEL.LE.7399)) DATAOUT=1

      IF (DATAOUT.EQ.1) THEN
        WRITE(6, *) 'Meta', kstep, kinc, noel, npt
        WRITE(6, *) 'STRAN', STRAN
...
CTOM --- If there's a huge strain increment, we're in X-FEM land and will encounter numerical dramas should we continue...
CTOM --- was it already failed before this started?
CTOM --- statev(126) - 
CTOM        0: OK
CTOM        1: Failed at STRAINRATE
CTOM        2: Failed at WORKST(I,J)
CTOM        3: Failed from too large a strain increment



        IF (STATEV(1).EQ.0.) STATEV(126)=0.

C        DO I=1,3
C            TRIAL=(STRAN(I) + DSTRAN(I))
C            IF (TRIAL.GT.STATEV(126)) STATEV(126)=TRIAL
C        END DO

CTOM -- If we haven't already written a gauss point off, check the strains aren't
C       too crazy
      IF (STATEV(126).EQ.0.) THEN
        DO I=1,3
            IF (DSTRAN(I).GT.0.05) THEN
                STATEV(126)=3.
                 WRITE (6, *) 'Excessive Strain Increment', kinc, noel, npt
                 WRITE (6, *), DSTRAN
            END IF
        END DO
      END IF

C-----  Jump back here and start again if there was some drama with numerical stuff
C ---   At this point "Failed" might have been changed.
500   CONTINUE
...
CTOM -- Get out of here and try again, assuming the material is done for.
      IF (UNSTBL.EQ.1.) THEN
        WRITE(6, *) 'Unstable STRAINRATE', kstep,kinc , npt, noel
        IF (STATEV(126).NE.0.) PAUSE 'Unstable STRAINRATE but already failed?'
        STATEV(126)=1.
        GO TO 500
      END IF
...
C
      ...
      TERM1=THETA*DTIME
C      WRITE(6,*) THETA, DTIME, TERM1, noel,npt,kstep,kinc
      DO I=1,NSLPTL
         ...
         DO J=1,NSLPTL
            ...
            IF (WORKST(I,J) .NE. WORKST(I,J)) THEN
               WRITE(6,*) 'WORKST(I,J)', I, J, WORKST(I,J)
               WRITE(6,*) TERM2, TERM4, H(I,J), TERM3, DSIGN(1.D0,FSLIP(J))
               WRITE(6,*) TERM1, DFDXSP(I), GSLIP, DHDGDG(I,J)
               WRITE(6,*) THETA, DTIME
               UNSTBL=1.
            END IF
...
CTOM -- Get out of here and try again, assuming the material is done for.
      IF (UNSTBL.EQ.1.) THEN
        IF (STATEV(126).NE.0.) PAUSE 'Unstable WORKST but already failed?'
        WRITE(6, *) 'Unstable ', kstep,kinc , npt, noel
        STATEV(126)=2.
        GO TO 500
      END IF
...
CTOM -- Only do this if we aer not all XFEMed out
        IF (STATEV(126).NE.0.) THEN
            DGAMMA(I)=0.
        ELSE
         IF (NITRTN.EQ.0) THEN
            TAUSLP=STATEV(2*NSLPTL+I)
...
CTOM--  Cap the shear stresses at some point - with XFEM they go a bit nuts
C      DO I=1,NSLPTL
C        IF (STATEV(2*NSLPTL+I).GT.1.D10) STATEV(2*NSLPTL+I) = 1.D10
C        IF (STATEV(2*NSLPTL+I).LT.-1.D10) STATEV(2*NSLPTL+I) = -1.D10
C        IF (STATEV(2*NSLPTL+I).NE.STATEV(2*NSLPTL+I)) THEN
C            WRITE(6, *) 'NanTau', I, STATEV(2*NSLPTL+I), DTAUSP(I), DTAUOD(I)
C            STATEV(2*NSLPTL+I) = 0.0
C        END IF
C      END DO
...
C----- Tom new bit start
C      Total energy seems to work.
      ETOTAL=0.0
      DO I=1,NTENS
        ETOTAL=ETOTAL+ 0.5*(STRESS(I) + OLDSIG(I)) * DSTRAN(I)
      END DO
      

C----- TAUSLP -> resolved shear stress in slip system (which has been updated by this point)
C----- DTAUSP -> increment of resolved shear stress in slip systems
C----- DGAMMA -> increment of strain in slip systems
      EPLAST=0.0
      DO I=1,NSLPTL
         TAUSLP=STATEV(2*NSLPTL+I)
         EPLAST=EPLAST + (TAUSLP - 0.5*DTAUSP(I)) *DGAMMA(I)
      END DO

      EELAST=ETOTAL-EPLAST
...
C TOMAAA DFDXSP getting infinite
        IF (X.GT.1.D6) THEN
        WRITE(6, *) 'sr', I, TAUSLP(I), GSLIP(I), X, FSLIP(I), DFDXSP(I)
        UNSTBL=1.
        END IF
        IF (X.NE.X) THEN
        WRITE(6, *) 'sr', I, TAUSLP(I), GSLIP(I), X, FSLIP(I), DFDXSP(I)   
...
C
           ...
           IF (F.GT.1.D200) THEN
C       TOM Addition - cap this if it overflows.
              F=1.D200
           END IF
           RETURN
           END
...
C
      ...
      SUBROUTINE LINELA (ntens, ddsdde, stress, dstran, ndi, ym)
      ...
      dimension stress(ntens),ddsdde(ntens,ntens),dstran(ndi)
...
C Lame's parameters
      lambda = ym*n/((1.0d0+n)*(1.0d0-2.0d0*n))
      mu = ym/(2.0d0*(1.0d0+n))
...
C Damage stuff (initiation and direction).
C Recap:
...
C If no slip has happened, don't need to do anything else!
         IF (NSLPTL.EQ.0) RETURN
C 
C Failure in a given plane is the ratio of plastic strain to crit value.
C First step is to find the slip plane with the max strain.
C
        CRITSTRN = PROPS(1)
        KMAX = 1
        SYSSTRN=STATEV(9*NSLPTL+1)
        FINDEX(1) = SYSSTRN/CRITSTRN
       DO K1 = 1, NSLPTL
         SYSSTRN=STATEV(9*NSLPTL+K1)
C        WRITE (6, *) 'Strain', K1, NSLPTL, 9*NSLPTL+K1, SYSSTRN, CRITSTRN
C        WRITE (6, *) STATEV
         
C DEBUG 
        IF (SYSSTRN/CRITSTRN.GT.1.0) THEN
            IF (FINDEX(1).LT.-1.0) THEN
                WRITE(6, *) 'XFEM ', K1, NOEL, NPT, SYSSTRN
                WRITE(6, *) 'Plane Normal X ', STATEV(3*NSLPTL+3*(K1-1)+1)
                WRITE(6, *) 'Plane Normal Y ', STATEV(3*NSLPTL+3*(K1-1)+2)
                WRITE(6, *) 'Plane Normal Z ', STATEV(3*NSLPTL+3*(K1-1)+2)
            END IF
        END IF
        
         IF (SYSSTRN/CRITSTRN.GT.FINDEX(1)) THEN
            FINDEX(1) = SYSSTRN/CRITSTRN
            KMAX=K1
        END IF
       END DO
C   
C Assign the plane normal from the slip plane with the most strain
C        
         DO K2=1,3
C  3*NSLPTL+1 should be the first index with KMAX=K2=1
            FNORMAL(K2,1)=STATEV(3*NSLPTL+3*(KMAX-1)+K2)
         END DO
