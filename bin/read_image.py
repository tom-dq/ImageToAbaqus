...
if __name__ == "__main__":
    ...
    # show the result
    if SHOW_IMAGES:
        plt_image = Image.open(FN)

        cv2.imshow("orig", img)
        cv2.imwrite("docs/mean_shift_starting.png", img)

        arr_orig = image_to_array(plt_image)

        plt.imshow(arr_orig, interpolation="nearest")

    bandwidth = 75
    shifted, raw_labels = mean_shift(bandwidth, 2000, img)

    if SHOW_IMAGES:
        cv2.imshow("shifted", shifted)
        cv2.imwrite(f"docs/mean_shift_after-{bandwidth}.png", shifted)
